...
/**
 * Adds an employee to a company's roster via POST /company/employees. An
 * HR record only: no User is created and no invite is issued this
 * phase. That arrives with the employee portal, the way a merchant's
 * team arrived after its profile.
 *
 ...
 * agree. The payload comes from StoreEmployeeRequest::payload(), never
 * raw request input, so spreading it into create() is safe: nothing in
 * it is outside $fillable, and company_id/status are set here, not by
 * the caller.
 */
