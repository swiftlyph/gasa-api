...
|--------------------------------------------------------------------------
|
| The second tenant type. BelongsToCompany is BelongsToMerchant's twin,
| and this block is the merchant block's twin: it runs against the real
| employees table (the first company-owned table) rather than a fixture,
| and asserts isolation through the HTTP endpoints a client would use.
*/

/**
 * @return array{0: User, 1: Company, 2: User, 3: Company}
 */
function twoCompaniesWithAnEmployeeEach(): array
{
    $adminOne = User::factory()->withRole('company_admin')->create();
    $companyOne = Company::factory()->ownedBy($adminOne)->create(['name' => 'Company One']);
    Employee::factory()->forCompany($companyOne)->create(['last_name' => 'Owned by One']);

    $adminTwo = User::factory()->withRole('company_admin')->create();
    $companyTwo = Company::factory()->ownedBy($adminTwo)->create(['name' => 'Company Two']);
    Employee::factory()->forCompany($companyTwo)->create(['last_name' => 'Owned by Two']);

    return [$adminOne, $companyOne, $adminTwo, $companyTwo];
}

test('a company admin only sees their own company\'s employees', function () {
    [, , $adminTwo] = twoCompaniesWithAnEmployeeEach();

    $list = $this->withToken($adminTwo->createToken('company')->plainTextToken)
        ->getJson('/api/v1/company/employees')->assertOk();

    expect($list->json('data'))->toHaveCount(1)
        ->and($list->json('data.0.last_name'))->toBe('Owned by Two');
});

test('fetching another company\'s employee by id is a 404, never a 403', function () {
    [, $companyOne, $adminTwo] = twoCompaniesWithAnEmployeeEach();

    $foreignId = DB::table('employees')->where('company_id', $companyOne->id)->value('id');

    $this->withToken($adminTwo->createToken('company')->plainTextToken)
        ->getJson("/api/v1/company/employees/{$foreignId}")
        ->assertStatus(404)
        ->assertJson(['code' => 'not_found']);
});

test('a spoofed company_id in the payload is ignored on employee create', function () {
    [$adminOne, $companyOne, , $companyTwo] = twoCompaniesWithAnEmployeeEach();

    $this->actingAs($adminOne->fresh());
    ...
    $employee = Employee::create([
        ...
        'first_name' => 'Cross',
        'last_name' => 'Tenant',
        'email' => 'cross.tenant@companyone.test',
    ]);

    expect($employee->company_id)->toBe($companyOne->id)
        ->and(DB::table('employees')->where('id', $employee->id)->value('company_id'))->toBe($companyOne->id);
});

test('a user with no active company matches no employees, not all of them', function () {
    [$adminOne, $companyOne] = twoCompaniesWithAnEmployeeEach();

    // A merchant has no company at all.
    $this->actingAs($this->merchantOneUser);
    expect(Employee::count())->toBe(0);

    // A company admin whose company is suspended has no ACTIVE company.
    $companyOne->update(['status' => 'suspended']);
    $this->actingAs($adminOne->fresh());
    expect(Employee::count())->toBe(0);
});

test('an unauthenticated context matches no employees', function () {
    twoCompaniesWithAnEmployeeEach();

    expect(Employee::count())->toBe(0);
});

test('a platform admin bypasses the company scope in admin context', function () {
    twoCompaniesWithAnEmployeeEach();

    // Same test-only probe technique as the merchant case above: proves
    // the bypass comes from admin.api's AllowsAdminContext, not the role.
    Route::prefix('api/v1/admin')
        ->middleware(['api', 'admin.api'])
        ->get('/company-tenancy-probe', fn () => response()->json([
    ...
    $this->withToken($admin->createToken('admin')->plainTextToken)
        ->getJson('/api/v1/admin/company-tenancy-probe')
        ->assertOk()
        ->assertJsonPath('employees', 2);
});

test('the same admin token on a company route is still 403 from the role middleware', function () {
    $admin = User::factory()->withRole('platform_admin')->create();
...
test('a merchant token on a company route is 403 from the role middleware, and the reverse holds too', function () {
    [$adminOne] = twoCompaniesWithAnEmployeeEach();
